...
- [ ] Worker registration and onboarding flow with zone and shift declaration
- [ ] Income baseline computation engine — 4-week rolling model with tolerance band
- [ ] Weekly premium engine — Random Forest risk scoring with 5 input features
- [ ] 5 parametric triggers connected to mock/live APIs with multi-signal validation
- [ ] AS scoring pipeline — device motion, network, platform activity, environmental, behavioral signals
- [ ] Claim auto-initiation and Soft Hold UX flow
- [ ] Razorpay test mode + UPI simulator integration for sub-5-minute payout demo
- [ ] Policy management module (Monday activation, Sunday expiry)

### Phase 3 — Scale and Optimise (April 5 – 17)

- [ ] Full 3-layer fraud detection with fraud ring detection (DBSCAN clustering)
- [ ] Worker dashboard — earnings protected, active coverage, disruption history
- [ ] Insurer admin dashboard — loss ratios, AS distribution, syndicate alert queue
- [ ] 6-hour ahead disruption push alerts based on forecast API data
- [ ] Final demo build — simulated live disruption, AS scoring, auto-payout walkthrough
